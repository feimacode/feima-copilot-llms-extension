...
Provides a single "Auto" entry in the Copilot Chat model picker that routes each request to the best available candidate from the existing local/enterprise endpoint pool, according to a user-selected routing strategy, without duplicating any endpoint or streaming logic owned by `LocalEndpointProvider`.
