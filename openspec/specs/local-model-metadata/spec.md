...
Resolves model capability metadata (context window, tool-calling, vision support) for discovered models, preferring endpoint-reported data over inference, falling back to a community-maintained reference table, and surfacing confidence (confirmed vs. estimated) in the UI.
