...
export function buildDisclosurePrefix(outcome: AutoOutcome): string {
	...
	return `> ${icon} **Auto** routed to *${outcome.candidate.info.name}* — ${outcome.reason}\n\n`;
}
...
export function buildFallbackMessage(outcome: Extract<AutoOutcome, { kind: 'fallback' }>): string {
	return `🧭 **Auto** could not find a qualifying local or enterprise endpoint for this request: ${outcome.reason}. `
		+ `Register an endpoint (\`Feima: Add Model Endpoint\`) or pick a model directly from the picker instead of Auto.`;
}
